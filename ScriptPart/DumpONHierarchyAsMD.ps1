...
function DumpNode($node) {
	switch ($node.Type) {
		"OutlineElement" {
			...
			$listItem = if ($node.ListItem) {
				switch ($node.ListItem.Type) {
					([Sidenote.DOM.ListItemType]::BulletListItem) { "- " }
					([Sidenote.DOM.ListItemType]::NumberedListItem) { "$($node.ListItem.Text) "  }
				}
			} else {
				""
			}

			Write-Output "$indent$listItem$($node.Text)"
			break
		}
